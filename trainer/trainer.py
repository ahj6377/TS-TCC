...
def model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config, device, training_mode):
    ...
    for batch_idx, (data, labels, aug1, aug2) in enumerate(train_loader):
        ...
        if training_mode == "self_supervised":
            ...
            temp_cont_loss1, temp_cont_lstm_feat1, tloss11, f11 = temporal_contr_model(features1, features2)
            temp_cont_loss2, temp_cont_lstm_feat2, tloss22, f22 = temporal_contr_model(features2, features1)
        ...
        if training_mode == "self_supervised":
            ...
            loss = (temp_cont_loss1 + temp_cont_loss2) * lambda1 + (tloss11 + tloss22) * lambda11 + ( 0.5 * nt_xent_criterion(zis, zjs) + 0.5 * nt_xent_criterion(f11,f22)) * lambda2
...
def Trainer_three(model,temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device, logger, config, experiment_log_dir, training_mode):
    # Start training
    logger.debug("Training started ....")
    ...
    for epoch in range(1, config.num_epoch + 1):
        ...
        if 1<= epoch and epoch <= 5:
            ...
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(model_optimizer, 'min')
        """
        train_loss, train_acc = model_train_three(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_dl, config, device, training_mode)
        valid_loss, valid_acc, _, _ = model_evaluate_three(model, temporal_contr_model, valid_dl, device, training_mode)
        if training_mode != 'self_supervised':  # use scheduler in all other modes.
            scheduler.step(valid_loss)
...
def model_train_three(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config, device, training_mode):
    total_loss = []
    total_acc = []
    model.train()
    temporal_contr_model.train()
...
def model_evaluate_three(model, temporal_contr_model, test_dl, device, training_mode):
    model.eval()
    temporal_contr_model.eval()
